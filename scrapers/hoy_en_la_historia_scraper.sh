...
# Insert a new line before either a 2 to 4 digit number followed by a space or by a 2 to 4 digit number, followed by "a.C. " in data.txt
...
grep -v -e 'See All' -e 'SHOW' -e 'Efemérides' "$dir"/data.txt | grep -vE '^.{,80}$' > "$dir"/hoy_en_la_historia.txt

# Replace ' - ' with ', ' in hoy_en_la_historia.txt
# sed -i 's/ - /, /g' "$dir"/hoy_en_la_historia.txt
sed -i -E 's/([0-9]{2,4}) /\1, /g' "$dir"/hoy_en_la_historia.txt
...
# sed -i -E 's/\.[^.]*$/\./' "$dir"/hoy_en_la_historia.txt
