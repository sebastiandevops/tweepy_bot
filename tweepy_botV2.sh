#!/usr/bin/env bash
# Secuence
echo $(curl --silent https://www.historynet.com/today-in-history/ | htmlq --text | html2text) | tr -s ' ' | sed '/./G' > data.txt
sed -r 's/(.)([0-9]{4}\s)/\1\n\2/g' data.txt > processed.txt
grep -E '[0-9]{4}\s' processed.txt > processed2.txt
cat processed2.txt | egrep -o '^[^.]+' > processed3.txt
grep -v -e '\[' -e '\]' -e '(' -e 'Lottery' -e 'See more' processed3.txt | grep -vE '^.{,30}$' > processed4.txt
